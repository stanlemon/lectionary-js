...
describe("CalendarBuilder", () => {
  ...
  it("month with 28 days starting on Sunday fills exactly 4 rows with no nulls", () => {
    ...
    expect(DateTime.local(2015, 2, 1).hasSame(grid[0][0].date, "day")).toBe(true);

    // Last cell is Feb 28, no trailing nulls
    expect(DateTime.local(2015, 2, 28).hasSame(grid[3][6].date, "day")).toBe(true);

    // Every cell in the grid is populated
    grid.forEach((week) => week.forEach((day) => expect(day).not.toBeNull()));
  });
