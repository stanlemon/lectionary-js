...
describe("YearFactory", () => {
  ...
  it("normalizes date-like input by year", () => {
    ...
    expect(byDateTime).toBe(YearFactory.get(2025, Year));
    expect(byDate).toBe(YearFactory.get(2025, Year));
    expect(byObject).toBe(YearFactory.get(2025, Year));
  });

  it("memoizes method calls on cached instances", () => {
    let calls = 0;
  ...
  it("preserves public Year semantics on frozen cached instances", () => {
    const year = YearFactory.get(2025, Year);
